...
The editor toolbar has a **Script**/**REPL** mode toggle. Script is the stateless behavior above. REPL turns the editor into a real REPL: each submitted line evaluates against a durable, per-browser Playground session (see [Sessions](#sessions)) and its result appears immediately below it in a scrolling log, so top-level state and a `/workspace` directory persist between lines; the result pane then also shows session status (executions, cwd, snapshot size, time until idle expiry) and a Workspace tab for browsing/editing/deleting files. Ruby has no REPL mode, since sessions aren't supported for Ruby.
