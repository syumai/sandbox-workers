...
- [Code interpreter](/api/interpreter) — `sandbox.interpreter.*`: `createCodeContext()`, `listCodeContexts()`, `deleteCodeContext()`, `runCode()`, plus `sandbox.setEnvVars()` and the free stateless `runCode()`
...
- [HTTP API](/api/http-api) — the raw JSON contract behind the client, on both sides of the wire
