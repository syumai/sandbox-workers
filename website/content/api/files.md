...
The File API can be disabled per Worker with `SANDBOX_FILE_API=disabled` (see [Environment variables](/configuration/environment-variables#sandbox_file_api--your-own-worker)); when it is, every method on this page throws `NotSupportedError` instead of running, and guest code gets `EACCES` on `/workspace` — see [Errors](/api/errors).
