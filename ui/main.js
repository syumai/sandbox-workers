...
// maps each language to one Playground-generated sandbox id, persisted
// (under `sandboxIds`; the old `sessionIds` key is ignored) so the same
// browser reuses it across visits. `cells` maps a session id to its last
// MAX_CELLS REPL log entries: {id, code, status, response, resultText,
...
let sessionIds =
  saved?.sandboxIds &&
  typeof saved.sandboxIds === "object" &&
  !Array.isArray(saved.sandboxIds)
    ? { ...saved.sandboxIds }
    : {};
let cellSeq = 0;
let cells = {};
...
async function runReplCell(code) {
  ...
  const url = `${sessionBaseUrl(lang, sid)}/execute`;
  try {
    const res = await fetch(url, {
      method: "POST",
      headers: { "content-type": "application/json" },
...
$("session-reset").onclick = async () => {
  ...
  const sid = sessionIdFor(lang);
  try {
    const res = await fetch(`${sessionBaseUrl(lang, sid)}/contexts`);
    const { contexts } = await res.json();
    for (const context of contexts ?? [])
      await fetch(
        `${sessionBaseUrl(lang, sid)}/contexts/${encodeURIComponent(context.id)}`,
        { method: "DELETE" },
      );
  } catch {}
  cells[sid] = [];
...
function renderWorkspaceList(entries) {
  ...
  for (const entry of entries) {
    ...
    name.textContent =
      entry.absolutePath + (entry.type === "directory" ? "/" : "");
    if (entry.type === "file")
      name.onclick = () => openWorkspaceFile(entry.absolutePath);
