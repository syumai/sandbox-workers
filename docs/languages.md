...
`POST /execute` boots a fresh Wasm instance every call. A **sandbox** is a `Sandbox` Durable Object hosted by the caller's own Worker instead, owning a shared `/workspace` and one or more named **code contexts**, each bound to a runtime Worker by the name of a Service Binding — durable, stateful REPLs: top-level variables, functions, and imported modules persist across calls to the same context, and survive Durable Object eviction, hibernation, and redeploys via a linear-memory snapshot taken after each execution on that binding's `Interpreter` Durable Object (see `docs/sandbox-1-0-design.md`, `docs/sessions-design.md`, and the [sandboxes and code contexts guide](../website/content/guides/code-contexts.md) for when a snapshot is skipped and how deleting a context compacts one). Code contexts are supported for JavaScript, Python, and Perl; a Ruby runtime Worker (or any runtime Worker deployed `--stateless`) reports `contexts: false` from `GET /interpreter`, and every `/interpreters/:key/*` route on it answers 400, except the context-less `POST /execute`, which runs statelessly. See the guide for the client API, the files API, and per-language REPL semantics.
