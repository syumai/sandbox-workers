...
Python, Perl, and Ruby have independent `packages/<language>` packages and Workers. See [language runtimes](languages.md) for engines, limits, and build details. All four engines but Ruby (which uses the official RubyVM ABI) share the same wasmify protobuf ABI described here.
