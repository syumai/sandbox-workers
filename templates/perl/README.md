...
Deploy this directory as a standalone project. It creates a private perl Worker for Service Bindings. A Workers Paid plan is required by the configured CPU limit.
...
Call `env.SANDBOX.fetch()` with a JSON POST to `https://sandbox.internal/execute` containing `{language: "perl", code, envVars}`. Code is a script: the value of the last expression is the result. Env vars (string values only) are available as `process.env.NAME` (JavaScript), `os.environ["NAME"]` (Python), `$ENV{NAME}` (Perl), or `ENV["NAME"]` (Ruby).
