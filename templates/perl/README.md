...
Call `env.SANDBOX.fetch()` with a JSON POST to `https://sandbox.internal/execute` containing `{language: "perl", code, input}`. Code is a function body; input is available as `input` (`$input` in Perl). Return a JSON-compatible value.
