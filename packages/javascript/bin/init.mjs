...
if (args[0] !== "init" || args.length > 2 || args[1]?.startsWith("-")) {
  console.log(
    "Usage: sandbox-workers-javascript init [directory]\nCreates a private Worker project. Then run npm install and npm run deploy in it.",
  );
  process.exit(args[0] === "--help" ? 0 : 1);
}
const directory = resolve(args[1] ?? "sandbox-javascript");
...
const files = {
  "package.json":
    JSON.stringify(
      {
        name: "sandbox-javascript-worker",
        private: true,
        type: "module",
        scripts: {
          dev: "wrangler dev",
        ...
        dependencies: { "@sandbox-workers/javascript": version },
        devDependencies: { wrangler: "^4.129.0" },
      },
      null,
      2,
    ) + "\n",
  "index.js": 'export { default } from "@sandbox-workers/javascript";\n',
  "wrangler.jsonc":
    JSON.stringify(
      {
