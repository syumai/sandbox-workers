...
if (args[0] !== "init" || args.length > 2 || args[1]?.startsWith("-")) {
  console.log(
    "Usage: sandbox-workers-perl init [directory]\nCreates a private Worker project. Then run npm install and npm run deploy in it.",
  );
  process.exit(args[0] === "--help" ? 0 : 1);
}
const directory = resolve(args[1] ?? "sandbox-perl");
    ...
    JSON.stringify(
      {
        ...
        dependencies: { "@sandbox-workers/perl": version },
        devDependencies: { wrangler: "^4.129.0" },
      },
      null,
      2,
    ) + "\n",
  "index.js": 'export { default } from "@sandbox-workers/perl";\n',
  "wrangler.jsonc":
    JSON.stringify(
      {
